<!DOCTYPE html>
<html>
<head>
   <title>اخر الأخبار</title>
    <meta charset="utf-8"/>
   <meta name="description" content="tanta"/>
    <meta name="viewport" content="width=device-width, intial-scale=1, shrink-to-fit=no"/>
    <link rel="stylesheet" href="css/bootstrap.min.css"/>
    <link rel="stylesheet" href="css/main.css"/>
    <link rel="shortcut icon" href="images/title.jpg" type="image/x-icon"/>     
    </head>
    <body>
   <div class="container">
        <nav class="navbar navbar-dark bg-dark">
  <img src="images/title.jpg" width="60" height="60" alt="" loading="lazy"/>
           <ul>
               <a class="navbar-brand" href="index.php">تسجيل الخروج</a>
               <a class="navbar-brand" href="about_us.php">About us</a>
            </ul>
        </nav>
        <div class="card">
  <h5 class="card-header">احمد سيد محمد</h5>
  <div class="card-body">
    <p class="card-text">مال</p>
   
  </div>
</div>
       <br/>
        <div class="card">
  <h5 class="card-header">خالد ابراهيم منتصر</h5>
  <div class="card-body">
    <p class="card-text">ادوات طبية</p>
   
  </div>
</div>      
       <br/>
       <br/>
       <div class="card">
  <div class="card-header">
    حذف عنصر
  </div>      
        <div class="card-body">
    <div class="form-group row">
        <label for="inputEmail3" class="col-sm-2 col-form-label">رقم العنصر</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="inputEmail3" autofocus />
                <button type="submit" class="btn btn-primary">حذف</button>
        </div>
  </div>
  </div>
</div>
       <footer style="background-color:#343A40"> All CopyRights&copy;Eslam Elbanna 2020</footer>
        </div>
    </body>
</html>
